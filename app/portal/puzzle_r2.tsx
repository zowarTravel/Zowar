  ...
  const check = React.useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/portal/check", {
        ...
        body: JSON.stringify({ round: "r2", answer, locale: safeLocale }),
      });
  ...
  if (locked) {
    return (
      <div className="rounded-3xl border border-neutral-200 bg-white p-5">
        <h2 className="text-xl font-semibold">{isAr ? "الجولة ٢ مقفلة" : "Round 2 Locked"}</h2>
        <p className="mt-2 text-neutral-700">{isAr ? "حل الجولة ١ أولاً لفتح هذا اللغز." : "Solve Round 1 first to unlock this riddle."}</p>
      </div>
    );
  }
