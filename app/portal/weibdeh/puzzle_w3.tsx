...
import { setWeibdehRoundSolved, serverSetWeibdehRoundSolved } from "./progress_weibdeh";
...
function Bubble({
  ...
  const base =
    "select-none rounded-full px-4 py-2.5 text-sm font-semibold transition active:scale-[0.98] border shadow-sm";
...
export default function PuzzleW3({
  locale,
  onSolved,
}: {
  locale: Locale;
  ...
  function check() {
    ...
    if (!solvedRef.current) {
      ...
      setWeibdehRoundSolved("w3");
      serverSetWeibdehRoundSolved("w3");
      onSolved?.();
    }
  }
          ...
          <div className="mt-6">
            ...
            <div
              ...
              className={[
                ...
                dragOverSolve ? "border-z-orange bg-orange-50" : "border-black/10 bg-white/85",
              ].join(" ")}
            >
              {pickedTiles.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {pickedTiles.map((tile, idx) => {
                    ...
                    const wrongAtPos =
                      checked && idx < targetNorm.length && builtNorm[idx] !== targetNorm[idx];
